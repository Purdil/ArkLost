...
{
    public class BossSkillModule : MonoBehaviour, IModule,ISkillModule
    {
        public ModuleOwner Owner { get; private set; }
        public AbstractBoss Enemy {get ; private set;}
        ...
        {
            Owner = owner;
            Enemy = owner as AbstractBoss;
            Debug.Assert(Enemy != null, $"적의 스킬 모듈은 반드시 AbstractBoss의 자식이어야 합니다. : {gameObject}");
            ...
            {
                skill.InitializeSkill(this); //각 스킬 초기화
            }
        ...
        public bool CanUseSkill(int skillIndex, GameObject target = null)
        {
            ...
            //움직임 체크
            if (_currentSkill is IMoveSkill moveSkill)
            {
                if(!moveSkill.CheckCanMoveSkill())
                    return false;
            }
        ...
        public void StopSkillIfNotFinished()
        {
            if (_currentSkill != null)
            {
                _currentSkill.StopSkill(); //이벤트랑 구독해체 다 이루어질거다.
                _currentSkill = null;
            }
        }
    }
